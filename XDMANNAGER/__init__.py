...
def get_user_list(__init__, key):
    with open("{}/XDMANNAGER/{}".format(os.getcwd(), __init__), "r") as json_file:
        return json.load(json_file)[key]


# enable logging
FORMAT = "[xᴅᴍᴀɴɴᴀɢᴇʀ] %(message)s"
logging.basicConfig(
...
LOGGER = logging.getLogger("[xᴅᴍᴀɴɴᴀɢᴇʀ]")
LOGGER.info("xᴅᴍᴀɴɴᴀɢᴇʀ ɪs sᴛᴀʀᴛɪɴɢ. | ᴀɴ xᴅ_ɴᴇᴛᴡᴏʀᴋ ᴘʀᴏᴊᴇᴄᴛ ᴘᴀʀᴛs. | ʟɪᴄᴇɴsᴇᴅ ᴜɴᴅᴇʀ ɢᴘʟᴠ3.")
...
MONGO_DB = "XDMANNAGER"  # DON'T EDIT
...
SPAMWATCH_SUPPORT_CHAT = "@XD_NETWORK"
...
try:
    ...
    raise Exception(
        "[xᴅᴍᴀɴɴᴀɢᴇʀ ᴇʀʀᴏʀ]: Your ʀᴇᴅɪs ᴅᴀᴛᴀʙᴀsᴇ ɪs ɴᴏᴛ ᴀʟɪᴠᴇ, ᴘʟᴇᴀsᴇ 𝐂𝐡𝐞𝐜𝐤 ᴀɢᴀɪɴ."
    )
finally:
    REDIS.ping()
...
from XDMANNAGER.modules.sql import SESSION

telegraph = Telegraph()
telegraph.create_account(short_name="XDMANNAGER")
...
async def get_entity(client, entity):
    ...
    if not isinstance(entity, Chat):
        ...
        try:
            ...
            for XDMANNAGER in apps:
                if XDMANNAGER != client:
                    try:
                        entity = await XDMANNAGER.get_chat(entity)
                    except (PeerIdInvalid, ChannelInvalid):
                        pass
                    else:
                        entity_client = XDMANNAGER
                        break
            else:
                entity = await XDMANNAGER.get_chat(entity)
                entity_client = XDMANNAGER
...
from XDMANNAGER.modules.helper_funcs.handlers import CustomMessageHandler, CustomRegexHandler

tg.RegexHandler = CustomRegexHandler
tg.MessageHandler = CustomMessageHandler

from XDMANNAGER.modules.helper_funcs.handlers import CustomCommandHandler
...
try:
    from XDMANNAGER.antispam import antispam_cek_user, antispam_restrict_user, detect_user
